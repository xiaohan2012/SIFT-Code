...
def gen_fp(receptor=None,binder=None,complex_id='',root_path=''):
    ...
    if not os.path.exists(os.path.abspath(os.path.join(root_path,complex_id))):
        os.mkdir(os.path.join(root_path,complex_id))

    o_fname = '%s/%s/%s_fp.dat' %(root_path,complex_id,complex_id)

    with open(o_fname,  'w') as out_fp:
